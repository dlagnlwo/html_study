...
const AbdB = {
    ...
    let i, no, str="", menu, brand1, menubar, mae, chang, comu, subbrand;
window.onload = function(){
    // 메뉴바
    // menu = document.querySelectorAll(".menu .brand").getElementsByClassName("brand");
    // macA.menuA.forEach((item, ind) => {
    //     menu[ind].innerHTML = macA.menuA[ind];
    // })

    // subbrand = document.querySelector('.brand_sub').getElementsByTagName("li");
    // ss=subbrand.getElementsByTagName("li");
    // bottom_nav = document.querySelector(".bottom_nav");
    // menu=bottom_nav.querySelector(".menu");
    // brand1=menu.querySelector(".brand");
    // brand_sub = brand1.getElementsByClassName(".brand_sub");
    // macA.brandA.forEach((item,ind)=>{
    //     brand_sub[ind].innerHTML = macA.brandA[ind];
    // });

    // let asd = document.querySelectorAll('.asd');
    // AbdB.asd.forEach((item,idx)=>{
    //     asd[idx].innerHTML=AbdB.asd[idx];
    // });
    
    let brand_sub;
    brand_sub = document.querySelectorAll(".brand_sub > a");
        macA.brandA.forEach((item, ind) => {
            brand_sub[ind].innerText = macA.brandA[ind];
        })
...
let timer=setInterval("show()", 4000);
    ...
    function show() {
        ...
        $(".btn p:eq(" + count + ")").css("color", "#E49292");
...
$(function() {
    ...
    $("#right_arrow").on("click", function() {
        ...
        $(".mid_img").animate({"left": pos + "px"},500, function(){
            $(".mid_img li:first-child").appendTo(".mid_img");
            $(".mid_img").css("left", "+=100%");
        });
    });
    $("#left_arrow").on("click", function() {
        ...
        console.log(count, pos);
        $(".mid_img").animate({"left": pos + "px"},500, function(){
            $(".mid_img li:first-child").prependTo(".mid_img");
            $(".mid_img").css("left", "+=100%");
        });
    });
    // $(".arrow p").on("click", function() {
    ...
    // left_menu exit버튼 누르면 나오고 닫기 누르면 숨겨짐
    ...
    // right_menu exit버튼 누르면 나오고 닫기 누르면 숨겨짐
    $(".top_nav div:nth-child(2)").on("click", function(){
        $(".right_menu").animate({"left": "1670px"});
    });
    $(".exit2 i").on("click", function(){
        $(".right_menu").animate({"left": "1920px"})
    })
